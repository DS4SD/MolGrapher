...
class GraphDataset(torch.utils.data.Dataset):
    ...
    def get_molecular_graph(self, image, keypoints, molecule):
        ...
        molecular_graph = MolecularGraph(self.config).from_rdkit_molecule(
            molecule, keypoints
        )
    ...
    def __getitem__(self, index, visualization=False):
        while True:
            if self.config["on_fly"]:
                smiles = pd.read_csv(
                    os.path.dirname(__file__) + "/../../data/smiles/experiment-052.csv",
                    header=None,
                    skiprows=index + 1,
                    nrows=1,
                )[5].values[0]
                ...
                keypoints = [
                    [keypoints_flat[i] - 1, keypoints_flat[i + 1] - 1]
            ...
            if len(keypoints) < 2:
                index = self.decrement_index(index)
                print(
                    "Dataset sanity check error: the image contains only 1 or no keypoints"
                )
                continue
            ...
            if not self.predict:
                ...
                transformed = self.transforms_dict["hard"](
            ...
            if self.hf_dataset:
                molecule = Chem.MolFromMolBlock(
                    self.dataset["mol"].iloc[index], sanitize=False, removeHs=False
                )
            else:
