...
class WAHRLoss(nn.Module):
    ...
    def forward(self, predicted_heatmaps, gt_heatmaps):
        weights = (gt_heatmaps**self.weighted_lambda) * torch.abs(
            1 - predicted_heatmaps
...
class KeypointDetector(pl.LightningModule):
    def __init__(self, config_dataset, config_training):
        ...
        self.nb_filters = list(self.feature_extractor.children())[-3][
            -1
        ].conv1.out_channels

        self.conv1 = nn.Conv2d(
            in_channels=self.nb_filters,
            out_channels=self.nb_filters // 2,
            kernel_size=(3, 3),
            padding=1,
        )
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.parameters(), lr=self.config_training["lr"]
        )  # AdamW
    ...
    def predict(self, images):
        ...
        for heatmap in predicted_heatmaps:
            ...
            Bc = np.ones((5, 5))
            heatmap_max = mahotas.regmax(
                heatmap_thresholded.numpy()[0, :, :], Bc=Bc
            ).astype(int)
            ...
            if bond_size > 35:
                Bc = np.ones((15, 15))
                heatmap_max = mahotas.regmax(
                    heatmap_thresholded.cpu().detach().numpy()[0, :, :], Bc=Bc
                ).astype(int)
                keypoints_y, keypoints_x = np.where(heatmap_max == 1)
                keypoints = [
                    [int(keypoint_x), int(keypoint_y)]
        ...
        return {
            ...
            "heatmaps_batch": [
                predicted_heatmap for predicted_heatmap in predicted_heatmaps
            ],
        }

    def predict_step(self, batch, batch_idx, drop_last=False):
        torch.set_num_threads(self.config_dataset["num_threads_pytorch"])
        return {"predictions_batch": self.predict(batch["images"]), "batch": batch}
