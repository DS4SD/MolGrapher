...
class MessagePassing(pl.LightningModule):
    def __init__(self, node_embedding_dimension):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.max_nb_neighbors, 128), nn.ReLU(), nn.Linear(128, 1)
        )
    ...
    def forward(self, data):
        ...
        for node_index, (central_node_embedding, node_type) in enumerate(
            zip(data.x, data.nodes_types)
        ):
            if node_type == 1:
                neighbors_nodes_embeddings = data.x[
                    neighboring_bonds_indices[node_index]
                ]
                    ...
                    neighbors_nodes_embeddings = torch.cat(
                        (
                            ...
                            torch.zeros(
                                ...
                                - neighbors_nodes_embeddings.size(0),
                ...
                global_consistency[node_index] = torch.dot(
                    constraint_embedding, central_node_embedding
                ) / (
                    torch.norm(constraint_embedding)
                    * torch.norm(central_node_embedding)
                )
...
class GNN(nn.Module):
    def __init__(self, config_dataset, node_embedding_dimension, gcn_on, device):
        ...
        if self.gcn_on:
            ...
            self.conv1 = GCNConv(
                self.node_embedding_dimension, self.node_embedding_dimension // 4
            )
            self.conv2 = GCNConv(
                self.node_embedding_dimension // 4, self.node_embedding_dimension // 8
            )
            self.conv3 = GCNConv(
                self.node_embedding_dimension // 8, self.node_embedding_dimension // 8
            )
            self.conv4 = GCNConv(
                self.node_embedding_dimension // 8, self.nb_filters_out
            )
            self.relu = nn.ReLU()
    ...
    def forward(self, data):
        ...
        atoms_indices = [
            index
            for index in range(len(data.nodes_types))
            if (data.nodes_types[index] == 1)
        ]
        bonds_indices = [
            index
            for index in range(len(data.nodes_types))
            if (data.nodes_types[index] == 0)
        ]
...
class GraphClassifier(pl.LightningModule):
    def __init__(self, config_dataset, config_training, gcn_on=False):
        ...
        self.gnn = GNN(
            config_dataset, self.node_embedding_dimension, gcn_on, self.device
        )
        ...
        if self.type_embedding:
            self.atom_type_embedding = nn.Parameter(
                torch.randn(self.node_embedding_dimension)
            )
            self.bond_type_embedding = nn.Parameter(
                torch.randn(self.node_embedding_dimension)
            )
    ...
    def predict(self, batch):
        ...
        predictions_atoms = [
            predicted_atom for predicted_atom in predictions[0].detach().cpu().tolist()
        ]
        predictions_bonds = [
            predicted_bond for predicted_bond in predictions[1].detach().cpu().tolist()
        ]
        ...
        for batch_index in range(batch["data"].num_graphs):
            ...
            predictions_atoms_sub = [
                np.argmax(prediction_atom)
                for prediction_atom in predictions_atoms[
                    offset_atoms : offset_atoms + nb_atoms_sub
                ]
            ]
            predictions_bonds_sub = [
                np.argmax(prediction_bond)
                for prediction_bond in predictions_bonds[
                    offset_bonds : offset_bonds + nb_bonds_sub
                ]
            ]
            confidences_atoms_sub = [
                torch.max(
                    softmax(torch.tensor(prediction_atom, requires_grad=False))
                ).item()
                for prediction_atom in predictions_atoms[
                    offset_atoms : offset_atoms + nb_atoms_sub
                ]
            ]
            confidences_bonds_sub = [
                torch.max(
                    softmax(torch.tensor(prediction_bond, requires_grad=False))
                ).item()
                for prediction_bond in predictions_bonds[
                    offset_bonds : offset_bonds + nb_bonds_sub
                ]
            ...
            confidences.append(
                round(min(confidences_atoms_sub + confidences_bonds_sub), 3)
            )
    ...
    def forward(self, batch):
        ...
        if self.positional_embedding:
            feature_maps = feature_maps + self.node_positional_embedding_map.repeat(
                self.config_dataset["batch_size"], 1, 1, 1
            )

        nodes_embeddings_init = torch.zeros(
            [batch["data"].num_nodes, self.node_embedding_dimension], device=self.device
        )
        i = 0
        ...
        for data_index in range(batch["data"].num_graphs):
            ...
            for node_index, node_type in enumerate(data_sub.nodes_types):
                if node_type == 1:
                    ...
                    sample_index += 1
                if node_type == 0:
                    map_node_sample_index[node_index] = list(
                        range(sample_index, sample_index + self.bond_nb_sampled_points)
                    )
            ...
            for node_index, (x, y) in enumerate(data_sub.nodes_positions):
                if data_sub.nodes_types[node_index] == 1:
                    nodes_grid_sub[0, map_node_sample_index[node_index], 0, 0] = (
                        2 * (x / (self.config_dataset["image_size"][1] - 1)) - 1
                    )
                    nodes_grid_sub[0, map_node_sample_index[node_index], 0, 1] = -(
                        2
                        * (
                            ((self.config_dataset["image_size"][2] - 1) - y)
                            / (self.config_dataset["image_size"][2] - 1)
                        )
                        - 1
                    )
                ...
                if data_sub.nodes_types[node_index] == 0:
                    bond_positions = [
                        data_sub.nodes_positions[
                            neighboring_atoms_indices[node_index][0]
                        ],
                        data_sub.nodes_positions[
                            neighboring_atoms_indices[node_index][1]
                        ],
                    ]
                    ...
                    bond_sampling_positions = [
                        get_point_of_trisection_coordinates(
                            bond_positions, ratio_b, ratio_e
                        )
                    ...
                    for (x_bond_sample, y_bond_sample), sample_index in zip(
                        bond_sampling_positions, map_node_sample_index[node_index]
                    ):
                        # Convert euclidian coordinate system to grid sample coordinate system
                        nodes_grid_sub[0, sample_index, 0, 0] = (
                            2
                            * (
                                x_bond_sample
                                / (self.config_dataset["image_size"][1] - 1)
                            )
                            - 1
                        )
                        nodes_grid_sub[0, sample_index, 0, 1] = -(
                            2
                            * (
                                (
                                    (self.config_dataset["image_size"][2] - 1)
            ...
            if len(bonds_image_features) > 0:
                if self.bond_fusion:
                    bonds_image_features = torch.stack(bonds_image_features).permute(
                        0, 2, 1
                    )
                    bonds_image_features = self.bond_fusor(bonds_image_features)[
                        :, :, 0
                    ]
                    # print(f"Bond fusor weights: {sum(self.bond_fusor[0].weight)}") # Debug. Check influence of each sampled point
                else:
                    bonds_image_features = torch.mean(
                        torch.stack(bonds_image_features), 1
                    )
            ...
            for node_index in range(data_sub.num_nodes):
                if data_sub.nodes_types[node_index] == 1:
                    ...
                    if self.type_embedding:
                        node_embedding_init = (
                            sampled_image_features[sample_index]
                ...
                if data_sub.nodes_types[node_index] == 0:
                    ...
                    if self.type_embedding:
                        node_embedding_init = (
                            bonds_image_features[bond_index] + self.bond_type_embedding
                        )
                    else:
            ...
            nodes_embeddings_init[i : i + data_sub.num_nodes, :] = (
                nodes_embeddings_init_sub
            )
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        atoms_indices = [
            index
            for index in range(len(batch["data"].nodes_types))
            if (batch["data"].nodes_types[index] == 1)
        ]
        bonds_indices = [
            index
            for index in range(len(batch["data"].nodes_types))
            if (batch["data"].nodes_types[index] == 0)
        ]

        loss_atoms = self.criterion_atoms(
            predictions[0].float(), batch["data"].nodes_classes[atoms_indices]
        )
        loss_bonds = self.criterion_bonds(
            predictions[1].float(), batch["data"].nodes_classes[bonds_indices]
        )
    ...
    def on_validation_epoch_end(self):
        ...
        if self.trainer.global_rank == 0:
            if (self.e2e_validation_step != 0) and (self.e2e_validation_step % 5 == 0):
                ...
                # Get last checkpoint
                graph_classifier_model_name = max(
                    glob.glob(
                        os.path.dirname(__file__)
                        + f"/../../data/models/graph_classifier/*"
                    ),
                    key=os.path.getctime,
                )
                evaluation_save_path = (
                    os.path.dirname(__file__)
                    + f"/../../data/scores/graph_classifier/{graph_classifier_model_name.split('/')[-1][:-5]}_{self.trainer.current_epoch}.json"
                )
