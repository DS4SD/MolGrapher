...
class GraphConstructor:
        ...
        if "mask_size" in self.config_dataset_keypoint:
            ...
            self.keypoints = [
                [
                    ...
                    int(keypoint[1] * scaling_factor + (scaling_factor // 2)),
                ]
    ...
    def test_point(self, x, y, vertices):
        ...
        is_right = [
            self.is_on_right_side(x, y, vertices[i], vertices[(i + 1) % num_vert])
    ...
    def _is_empty(self, bond):
        ...
        mask = np.zeros(
            (
                self.config_dataset_keypoint["image_size"][1],
                self.config_dataset_keypoint["image_size"][2],
                self.config_dataset_keypoint["image_size"][0],
            ),
            dtype=np.float,
        )
        mask = cv2.fillPoly(
            mask, pts=[np.int0(cv2.boxPoints(rectangle))], color=(1, 1, 1)
        )
        ...
        for index_keypoint_query, keypoint_query in enumerate(self.keypoints):
            ...
            for index_keypoint_key, keypoint_key in enumerate(self.keypoints):
                ...
                if index_keypoint_key < index_keypoint_query:
                    distance = np.sqrt(
                        (keypoint_query[0] - keypoint_key[0]) ** 2
                        + (keypoint_query[1] - keypoint_key[1]) ** 2
                    )
            ...
            if len(distances) > 0:
                ...
                indices_keypoint_neighbors = [
                    index_keypoint
                    for (distance, index_keypoint) in sorted(
                        zip(distances, indices_keypoints_keys), key=lambda x: x[0]
                    )[: self.max_number_neighbors]
                ]

                for index_keypoint_neighbor in indices_keypoint_neighbors:
                    self.bonds_candidates.append(
                        [index_keypoint_query, index_keypoint_neighbor]
                    )
    ...
    def augment_gt_molecular_graph(self):
        gt_bonds_indices = [
            set(bond["index"]) for bond in self.gt_molecular_graph.bonds
        ]
        for bond in self.bonds:
                ...
                self.gt_molecular_graph.bonds.append(
                    {
                        ...
                        "class": self.types_classes_bonds["None"],
                        "position": [
                            (self.keypoints[bond[0]][0] + self.keypoints[bond[1]][0])
                            / 2,
                            (self.keypoints[bond[0]][1] + self.keypoints[bond[1]][1])
                            / 2,
                        ],
                        "type": 0,
                    }
