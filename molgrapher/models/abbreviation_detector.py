...
def get_ocr_angle(force_cpu):
    ocr_angle = PaddleOCR(
        ...
        show_log=False,
        rec_image_inverse=False,
        det_model_dir=os.path.dirname(__file__)
        + "/../../data/external/paddleocr/en_PP-OCRv3_det_infer/",
        rec_model_dir=os.path.dirname(__file__)
        + "/../../data/external/paddleocr/en_PP-OCRv3_rec_infer/",
        cls_model_dir=os.path.dirname(__file__)
        + "/../../data/external/paddleocr/ch_ppocr_mobile_v2.0_cls_infer",
        use_gpu=not (force_cpu),
    )
    return ocr_angle
...
class AbbreviationDetector:
    ...
    def _detect_process(self, images_filenames_list, bonds_sizes_list):
        abbreviations_list = []
        for image_filename, bond_size in tqdm(
            zip(images_filenames_list, bonds_sizes_list), total=len(bonds_sizes_list)
        ):
    ...
    def mp_run(self, images_filenames, graphs, bonds_sizes, filter=False):
        if filter:
            ...
            for image_filename, bond_size, graph in zip(
                images_filenames, bonds_sizes, graphs
            ):
        ...
        if self.force_cpu and not (self.force_no_multiprocessing):
            ...
            bonds_sizes_split = np.array_split(
                bonds_sizes, self.config["num_processes_mp"]
            )
            args = [
                [
                    images_filenames_split[process_index],
                    bonds_sizes_split[process_index],
                ]
    ...
    def filter_image(
        ...
        if pr_mode == 1:
            ...
            t1 = cv2.morphologyEx(
                t1, cv2.MORPH_OPEN, np.ones((kernel_size, kernel_size))
            )
        else:
            t1 = cv2.morphologyEx(
                t1, cv2.MORPH_GRADIENT, np.ones((kernel_size, kernel_size))
            )

        img = cv2.cvtColor(np.zeros_like(t1), cv2.COLOR_GRAY2RGB)
        contours, _ = cv2.findContours(
            t1.astype(np.uint8), contour_method, contour_approx_mode
        )
        cnts = self.get_contours(
            contours, bond_size / 1.5, bond_size * 75, bond_size / 2.5
        )
    ...
    def get_contours(self, contours, beta1, beta2, alpha, beta=0.2):  # 0.2
        ...
        cnts = []
        bsd = lambda x: np.abs(
            np.linalg.norm(x[0] - x[1]) - np.linalg.norm(x[0] - x[3])
        )
        for cnt in contours:
            ...
            if (beta1 > area) or (area > beta2) or (bsd(box) > alpha):
                continue
            if not (rect[2] in [0, -90, 90, 180, -180, 270, -270, 360, -360]) and (
                min(rect[1][0], rect[1][1]) / max(rect[1][0], rect[1][1]) < beta
            ):
                continue
            cnts.append(cnt)
        return cnts

    def set_label_crops_and_boxes(
        self,
        scope=1.05,
        contour_method=cv2.RETR_EXTERNAL,
        contour_approx_mode=cv2.CHAIN_APPROX_SIMPLE,
        ...
        """
        contours, _ = cv2.findContours(
            self.filtered_image.astype(np.uint8), contour_method, contour_approx_mode
        )
        ...
        for cts in contours:
            ...
            min_x, max_x, min_y, max_y = (
                min([i[0] for i in box]),
                max([i[0] for i in box]),
                min([i[1] for i in box]),
                max([i[1] for i in box]),
            )
    ...
    def get_overlaps(self, boxes, anchor_box, index):
        ...
        for a in range(len(boxes)):
            if a == index:
                continue
            if (
                (boxes[a][0][0] >= anchor_box[1][0])
                or (boxes[a][1][0] <= anchor_box[0][0])
                or (boxes[a][0][1] >= anchor_box[1][1])
                or (boxes[a][1][1] <= anchor_box[0][1])
            ):
                continue
            overlaps.append(a)
        return overlaps
    ...
    def get_label_crops_predictions(
        self,
        display_paddle_predictions=False,
        display_paddle_predictions_individual=False,
    ):
        """
        Returns:
        ...
        for i in range(len(self.boxes)):
            ...
            if (
                self.label_crops[i].shape[0] < self.bond_size / 1.5
            ):  # and (self.label_crops[i].shape[0] < 0.4*self.bond_size):
                # Recognition only for small boxes
                predictions = self.ocr_recognition_only.ocr(
                    self.label_crops[i] * 255, det=False, rec=True, cls=False
                )
                ...
                predictions = self.ocr.ocr(
                    self.label_crops[i] * 255, det=True, rec=True, cls=False
                )[0]
                if not (predictions):
                    # Fallback option
                    predictions = self.ocr_recognition_only.ocr(
                ...
                for prediction in predictions:
                    ...
                    label_crop_boxes.append(
                        np.array(
                            [
                                [
                                    prediction[0][0][0] + self.boxes[i][0][0],
                                    prediction[0][0][1] + self.boxes[i][0][1],
                                ],
                                [
                                    prediction[0][0][0] + self.boxes[i][1][0],
                                    prediction[0][0][1] + self.boxes[i][1][1],
                                ],
                                [
                                    prediction[0][0][0] + self.boxes[i][2][0],
                                    prediction[0][0][1] + self.boxes[i][2][1],
                                ],
                                [
                                    prediction[0][0][0] + self.boxes[i][3][0],
                                    prediction[0][0][1] + self.boxes[i][3][1],
                                ],
                            ]
                        )
                    )
                if "S" in label_crop_texts and (
                    ("O2" in label_crop_texts)
                    or ("02" in label_crop_texts)
                    or ("Oz" in label_crop_texts)
                    or ("0z" in label_crop_texts)
                ):
                    if not (
                        all(label_crop_text in ["S", "O2", "02", "Oz", "0z"])
                        for label_crop_text in label_crop_texts
                    ):
                        continue
                    label_crop_texts = ["SO2"]
                    label_crop_scores = [1]
                    label_crop_boxes = [self.boxes[i]]
                if "C" in label_crop_texts and (
                    ("F2" in label_crop_texts) or ("Fz" in label_crop_texts)
                ):
                    if not (
                        all(label_crop_text in ["C", "F2", "Fz"])
                        for label_crop_text in label_crop_texts
                    ):
                        continue
...
class AbbreviationDetectorGPU(AbbreviationDetector):
    def __init__(
        ...
        super(AbbreviationDetectorGPU, self).__init__(
            config, image_size, force_cpu, force_no_multiprocessing, angle_recognition
        )
...
class AbbreviationDetectorCPU(AbbreviationDetector):
    ...
    def __init__(
        ...
        AbbreviationDetectorCPU.ocr_recognition_only = get_ocr_recognition_only(
            force_cpu=True
        )
        caption_remover = CaptionRemover(force_cpu=True)
        super(AbbreviationDetectorCPU, self).__init__(
            config, image_size, force_cpu, force_no_multiprocessing, angle_recognition
        )
...
class SpellingCorrector:
    ...
    def __call__(self, predicted_abbreviation):
        ...
        if len(indices_to_check) == 1:
            ...
            keys_candidates = [
                list(self.abbreviations.keys())[index] for index in indices_to_check
            ]
            abbreviations_populations = {
                k: v["population"]
                for k, v in self.abbreviations.items()
                if k in keys_candidates
            }
            max_population = 0
            ...
            print(
                f"Spelling correction problem: multiple abbreviation candidates for {predicted_abbreviation} have the same score: {keys_candidates}. {corrected_abbreviation} is chosen"
            )
