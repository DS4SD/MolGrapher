...
class GraphRecognizer(pl.LightningModule):
    ...
    def predict(
        self, batch, use_gt_structure=False, verbose=False, return_all_steps=False
    ):
        if verbose:
            print("--------------------------------------------------")
        ref_t = time()
        ...
        with torch.no_grad():
            if use_gt_structure:
                ...
                keypoints_batch, heatmaps_batch = (
                    keypoints_predictions["keypoints_batch"],
                    keypoints_predictions["heatmaps_batch"],
                )
            ...
            if verbose:
                print(
                    f"Dense graph construction completed in: {round(time() - ref_t, 2)}"
                )
            ref_t = time()

        self.graph_classifier.eval()
        with torch.no_grad():
            batch = {"images": images, "data": graphs_structures}
    ...
    def predict_step(self, batch, batch_idx, drop_last=False):
        ...
        if drop_last and (
            len(batch["images"]) < self.config_dataset_graph["batch_size"]
        ):
            print(
                f"The proposed batch size: {len(batch['images'])} is smaller than the configurated batch size: {self.config_dataset_graph['batch_size']}"
            )
            return None

        return {"predictions_batch": self.predict(batch), "batch": batch}
...
class StereochemistryRecognizer:
    ...
    def get_bond_direction(self, image, bond, atoms, bond_length):
        ...
        test_window_size = (int(0.3 * bond_length), int(0.3 * bond_length))
        center_up = (
        ...
        mask_up = torch.tensor(
            cv2.fillPoly(
                mask, pts=[np.int0(cv2.boxPoints(rectangle_up))], color=(1, 1, 1)
            )
        ).permute(2, 0, 1)
        ...
        mask_down = torch.tensor(
            cv2.fillPoly(
                mask, pts=[np.int0(cv2.boxPoints(rectangle_down))], color=(1, 1, 1)
            )
        ).permute(2, 0, 1)
        masked_image_down = (1 - image) * mask_down

        if (torch.sum(masked_image_up == True).item()) > (
            torch.sum(masked_image_down == True).item()
        ):
    ...
    def __call__(self, images, graphs, bonds_sizes):
        for bond_size, image, graph in zip(bonds_sizes, images, graphs):
            for bond in graph.bonds:
                if (self.bonds_classes_types[bond["class"]] == "DASHED") or (
                    self.bonds_classes_types[bond["class"]] == "SOLID"
                ):
                    bond["class"] = self.get_bond_direction(
                        image, bond, graph.atoms, bond_size
                    )
        return graphs
