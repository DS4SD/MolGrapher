...
def main():
    ...
    output_folder_path = (
        ...
        + f"/../../../data/visualization/predictions/keypoint_detector/jpo_10/"
    )
    visualize = True
    force_cpu = False
    ...
    for benchmark, nb_sample_benchmark in zip(
        config_dataset["benchmarks"], config_dataset["nb_sample_benchmarks"]
    ):
        # Read dataset
        if evaluate:
        ...
        model = KeypointDetector.load_from_checkpoint(
            os.path.dirname(__file__)
            + f"/../../../data/models/keypoint_detector/{keypoint_detector_model_name}.ckpt",
        ...
        predictions = [
            ...
            for predictions_batch in predictions_out
            if predictions_batch != None
        ]
        batches = [
            predictions_batch["batch"]
            for predictions_batch in predictions_out
            if predictions_batch != None
        ]
        ...
        images_filenames = [
            ...
            for image_filename in batch["images_filenames"]
        ]
        predicted_keypoints_list = [
            keypoints
            for prediction in predictions
            for keypoints in prediction["keypoints_batch"]
        ]
        predicted_heatmaps = [
            heatmap
            for prediction in predictions
            for heatmap in prediction["heatmaps_batch"]
        ]

        # Get bonds sizes
        predicted_bonds_sizes = [
            get_bond_size(keypoints) for keypoints in predicted_keypoints_list
        ]
        images_filenames_bonds_sizes = {
            k.split("/")[-1]: round(v, 2)
            for k, v in zip(images_filenames, predicted_bonds_sizes)
        }
        print("Bond sizes: ", images_filenames_bonds_sizes)
        with open(
            f"{config_dataset['benchmarks'][0]}_bonds_sizes.json", "w"
        ) as outfile:
        ...
        if visualize or evaluate:
            ...
            gt_keypoints_list = [
                gt_keypoints
                for batch in batches
                for gt_keypoints in batch["keypoints_batch"]
            ]

            scaling_factor = (
                config_dataset["image_size"][1] // config_dataset["mask_size"][1]
            )
            gt_keypoints_list = [
                [
                    [
                        gt_keypoint[0] * scaling_factor + scaling_factor // 2,
                        gt_keypoint[1] * scaling_factor + scaling_factor // 2,
                    ]
            ...
            predicted_keypoints_list = [
                [
                    [
                        predicted_keypoint[0] * scaling_factor + scaling_factor // 2,
                        predicted_keypoint[1] * scaling_factor + scaling_factor // 2,
                    ]
        ...
        if visualize:
            ...
            for (
                ...
                axis[0].scatter(*zip(*gt_keypoints), color="red", alpha=0.5)
                axis[1].imshow(predicted_heatmap.permute(1, 2, 0))

                axis[1].imshow(1 - image, alpha=0.25 * (image < 1), cmap="gray")
                axis[1].scatter(
                    *zip(*predicted_keypoints), color="white", s=2, alpha=0.5
                )
        ...
        if evaluate:
            ...
            for gt_keypoints, predicted_keypoints, image_filename in zip(
                gt_keypoints_list, predicted_keypoints_list, images_filenames
            ):
                precision, recall = get_metrics_keypoint_detector(
                    gt_keypoints, predicted_keypoints
                )
                if precision == None:
                    continue
            ...
            for image_name in scores.keys():
                if (
                    scores[image_name]["precision"] == 1
                    and scores[image_name]["recall"] == 1
                ):
            ...
            with open(
                os.path.dirname(__file__)
                + f"/../../../data/scores/keypoint_detector/{keypoint_detector_model_name}_{benchmark}.json",
                "w",
            ) as file:
