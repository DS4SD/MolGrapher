...
from molgrapher.models.abbreviation_detector import AbbreviationDetectorGPU, AbbreviationDetectorCPU, SpellingCorrector
...
def main():
    ...
    parser.add_argument('--benchmarks', nargs='+', help='<Required> Set flag', default=[])
    parser.add_argument('--nb-sample-benchmarks', nargs='+', help='<Required> Set flag', default=[])
    ...
    with open(os.path.dirname(__file__) + "/../../../data/config_training_graph.json") as file:
    ...
    if len(config_training_graph["devices"]) != 1:
        ...
        exit(0)
    if len(config_dataset_graph['benchmarks']) != len(config_dataset_graph['nb_sample_benchmarks']):
    ...
    for benchmark, nb_sample_benchmark in zip(config_dataset_graph['benchmarks'], config_dataset_graph["nb_sample_benchmarks"]):
        output_folder_path = os.path.dirname(__file__) + f"/../../../data/visualization/predictions/molecular_recognition/{benchmark}_{config_dataset_graph['experiment_name']}_{model_name}"
        ...
        for taa_step in range(number_taa_steps):
            ...
            if not saved_predictions:
                ...
                if save_predictions:
                    with open(os.path.dirname(__file__) + f"/../../../data/predictions/predictions_{config_dataset_graph['experiment_name']}_{benchmark}_{nb_sample_benchmark}.pickle", "wb") as f:
                        pickle.dump(predictions_out, f, protocol=pickle.HIGHEST_PROTOCOL)
            else:
                with open(os.path.dirname(__file__) + f"/../../../data/predictions/predictions_{config_dataset_graph['experiment_name']}_{benchmark}_{nb_sample_benchmark}.pickle", "rb") as f:
        ...
        if test_time_augmentation:
            ...
            for i in range(len(images_filenames)):
                confidences_predictions_i = [predictions_taa[taa_step]["confidences"][i] for taa_step in range(number_taa_steps)]
                best_candidate_index = confidences_predictions_i.index(max(confidences_predictions_i))
                predictions_reduced["graphs"].append(predictions_taa[best_candidate_index]["graphs"][i])
                predictions_reduced["keypoints"].append(predictions_taa[best_candidate_index]["keypoints"][i])
                predictions_reduced["confidences"].append(predictions_taa[best_candidate_index]["confidences"][i])
        ...
        if evaluate_graph_classifier:
            ...
            for predicted_graph, gt_graph in zip(predictions["graphs"], gt_graphs):
                ...
                gt_keypoints = []
                gt_classes = []
                for predicted_atom, gt_atom in zip(predicted_graph.atoms, gt_graph.atoms):
            ...
            print(f"Node Classifier Molecule Precision: {round(correct_node_prediction/total_node_prediction, 2)}")

        if visualize or evaluate or (visualize_all_steps and rdkit_output) or evaluate_superatom_ocr:
            # Compute bond size
            ...
            abbreviations_smiles_mapping = get_abbreviations_smiles_mapping(benchmark_dataset = benchmark, filtered_evaluation = filtered_evaluation)
            predicted_molecules = []
        ...
        if visualize_all_steps:
            ...
            predictions["heatmaps"] = [heatmaps for prediction in predictions_list for heatmaps in prediction["heatmaps_batch"]]
            predictions["heatmaps"] = [t(heatmap) for heatmap in predictions["heatmaps"]]
            
            if rdkit_output:
                for image_filename, image, graph, keypoints, predicted_heatmap, molecule in tqdm(zip(images_filenames, images, predictions["graphs"], predictions["keypoints"], predictions["heatmaps"], predictions["molecules"]), total=len(images_filenames)):
                    ...
                    plt.savefig(f"{output_folder_path}/{image_filename.split('/')[-1][:-4]}-1.png", dpi=500)
                    plt.close()

                    figure, axis = plt.subplots(1, 1, figsize=(7, 7))
                    graph.display_data_nodes_only(axis=axis, simple_display=True, supergraph=False)
                    plt.savefig(f"{output_folder_path}/{image_filename.split('/')[-1][:-4]}-2.png", dpi=500)
                    plt.close()
                    ...
                    plt.savefig(f"{output_folder_path}/{image_filename.split('/')[-1][:-4]}-4.png", dpi=500)
                    plt.close()
            else:
                for image_filename, image, graph, keypoints, predicted_heatmap in tqdm(zip(images_filenames, images, predictions["graphs"], predictions["keypoints"], predictions["heatmaps"]), total=len(images_filenames)):
                    ...
                    figure, axis = plt.subplots(1, 1, figsize=(8, 8))
                    graph.display_data_nodes_only(axis=axis, simple_display=False, supergraph=True)
                    plt.savefig(f"{output_folder_path}/{image_filename.split('/')[-1][:-4]}-3.png", dpi=500)
                    plt.close()
        ...
        if evaluate:
            ...
            detected_errors = 0
            for image_filename, predicted_molecule, molfile_filename in tqdm(zip(images_filenames, predictions["molecules"], molfiles_filenames), total=len(images_filenames)):
                ...
                if filtered_evaluation and ("jpo" not in benchmark):
                    ...
                    if molfile_annotator is None:
                        print(f"The MolFile annotation is not supported: {molfile_filename}")
                        continue
                    ...
                    if are_markush != [False] or (nb_structures != 1):
                        print(f"The MolFile contains multiple compounds or a markush structure: {molfile_filename}")
                        continue
                ...
                if filtered_no_charges_evaluation and ("jpo" not in benchmark):
                    ...
                    if (are_markush != [False]) or (nb_structures != 1) or (are_abbreviated != [False]) or (are_molecules != [True]):
                        print(f"The MolFile contains multiple compounds or a markush structure: {molfile_filename}")
                        continue
            ...
            try:
                detected_error_rate = round(detected_errors/sum([scores[i]['correct'] == False for i in  scores.keys()]), 4)
            except:
            ...
            scores_logging[benchmark] = {
                "keypoint_detector_model": model.keypoint_detector_model_name.split("/")[-1],
                "graph_classifier_model": model.graph_classifier_model_name.split("/")[-1],
        ...
        if evaluate_superatom_ocr:
            ...
            predicted_superatoms_list = [graph.superatoms for graph in predictions["graphs"]]
            print(predicted_superatoms_list)
            print(gt_superatoms_list)
            for i, (predicted_superatoms, gt_superatoms) in enumerate(zip(predicted_superatoms_list, gt_superatoms_list)):
                if gt_superatoms != []:
            ...
            if superatom_total == 0:
                ...
                print(f"Superatom recognition precision: {superatom_correct/superatom_total}")
    ...
    if args.evaluation_save_path == "":
        args.evaluation_save_path = os.path.dirname(__file__) + "/../../../data/scores/molecular_recognition/synthesis_" + model.graph_classifier_model_name.split('/')[-1] + \
            "_" + model.keypoint_detector_model_name.split('/')[-1] + "_" + str(config_dataset_graph['benchmarks']) + \
            "_" + str(config_dataset_graph['nb_sample_benchmarks']) + ".json"
