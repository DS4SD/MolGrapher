...
def main():
    ...
    precompute = False
    output_folder_path = os.path.dirname(__file__) + f"/../../../data/visualization/predictions_uspto_large_graph_classifier_aug_14/"
    ...
    model = GraphClassifier.load_from_checkpoint(
        os.path.dirname(__file__) + f"/../../../data/models/graph_classifier/{model_name}.ckpt", 
    ...
    predictions = [prediction for predictions_batch in predictions for prediction in predictions_batch]
    
    # Save predictions
    if not os.path.exists(output_folder_path):
        os.makedirs(output_folder_path)

    for index, (image, graph, gt_graph) in enumerate(zip(images, predictions, gt_graphs)):
