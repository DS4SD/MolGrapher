    ...
    for benchmark, nb_sample_benchmark in zip(
        ...
        if save:
            save_smiles_path = (
                ...
                + f"/../../../data/predictions/{model_name}/{benchmark}_{nb_sample_benchmark}.csv"
            )

        if visualize or visualize_outliers:
            output_folder_path = (
                os.path.dirname(__file__)
                + f"/../../../data/visualization/predictions/{model_name}/{benchmark}/"
            )
        ...
        if model_name == "molscribe":
            ...
            molfiles_filenames = [
                molfile_filename
                for batch in loader
                for molfile_filename in batch["molfiles_filenames"]
            ]
        ...
        if visualize:
            ...
            for image_filename, smiles in tqdm(
                zip(images_filenames, predicted_smiles_list),
                ...
                plt.savefig(
                    f"{output_folder_path}/{image_filename.split('/')[-1]}", dpi=500
                )
                plt.close()
        ...
        if evaluate:
            ...
            for image_filename, predicted_smiles, molfile_filename in zip(
                images_filenames, predicted_smiles_list, molfiles_filenames
            ):
        ...
        if visualize_outliers:
            ...
            for image_filename, predicted_smiles in tqdm(
                zip(images_filenames, predicted_smiles_list),
                total=len(images_filenames),
            ):
                image_name = image_filename.split("/")[-1][:-4]
                ...
                if (predicted_smiles is None) or (
                    Chem.MolFromSmiles(predicted_smiles) is None
                ):
