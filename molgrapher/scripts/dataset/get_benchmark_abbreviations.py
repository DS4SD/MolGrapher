...
for molfile in glob.glob(
    os.path.dirname(__file__) + f"/../../../data/benchmarks/{benchmark}/molfiles/*"
):
    ...
    if (
        molfile_annotator.rdk_mol
        and molfile_annotator.mol
        and molfile_annotator.mol["sgroups"]
    ):
        ...
        for sgroup in sgroups:
            ...
            for sgroup_index in sgroup_indices:
                for bond in molfile_annotator.rdk_mol.GetAtomWithIdx(
                    sgroup_index
                ).GetBonds():
            ...
            if (
                ...
                or (label == "I")
                or (label == "j2")
                or any([ord(e) == 65533 for e in label])
            ):
                continue

            if ("." in sgroup_smiles) or all(
                [c in ["*", "(", ")"] for c in sgroup_smiles]
            ):
                continue
