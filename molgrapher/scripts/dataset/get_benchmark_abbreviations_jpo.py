...
for molfile in glob.glob(
    os.path.dirname(__file__) + "/../../../data/benchmarks/jpo/molfiles/*"
):
    ...
    try:
        condensed_molecule = rdAbbreviations.CondenseAbbreviationSubstanceGroups(
            molecule
        )
    except:
        continue
...
abbreviations_population_sorted = dict(
    sorted(abbreviations_population.items(), key=lambda item: item[1])
)
...
for molfile in glob.glob(
            ...
            for bond in molecule.GetAtomWithIdx(sgroup_index).GetBonds():
                for neighbor_index in [
                    ...
                    if neighbor_index not in sgroup["indices"]:
                        try:
                            abbreviated_atom = condensed_molecule.GetAtomWithIdx(
                                neighbor_index
                            )
                        except:
                        ...
                        if (
                            abbreviated_atom.GetSymbol() == "*"
                        ) and abbreviated_atom.HasProp("atomLabel"):
    ...
    for sgroup in sgroups:
        ...
        if "smt_label" in sgroup:
            abbreviations_smiles[sgroup["smt_label"]].append(
                Chem.MolToSmiles(sgroup_molecule)
            )

abbreviations_smiles_unique = {
    k: {"population": abbreviations_population_sorted[k], "smiles": list(np.unique(v))}
    for k, v in abbreviations_smiles.items()
}
abbreviations_smiles_unique_sorted = dict(
    sorted(
        abbreviations_smiles_unique.items(),
