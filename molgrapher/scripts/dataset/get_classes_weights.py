...
def main():
    ...
    atoms_symbols_population = {
        atoms_classes_symbols[k]: v for k, v in atoms_classes_population.items()
    }
    bonds_symbols_population = {
        bonds_classes_types[k]: v for k, v in bonds_classes_population.items()
    }
    ...
    atoms_weights = {
        k: 1 if v > atoms_threshold else 3 for k, v in atoms_symbols_population.items()
    }
    bonds_weights = {
        k: 1 if v > bonds_threshold else 3 for k, v in bonds_symbols_population.items()
    }

    with open(
        f"{config_dataset['experiment_name']}_atoms_weights.json", "w"
    ) as outfile:
        json.dump(atoms_weights, outfile)

    with open(
        f"{config_dataset['experiment_name']}_bonds_weights.json", "w"
    ) as outfile:
