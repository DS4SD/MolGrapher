...
def main():
    ...
    for image_name in molecular_information.keys():
        molfile_filename = (
            os.path.dirname(__file__)
            + f"/../../../data/benchmarks/uspto_large/molfiles/{image_name}.MOL"
        )
    ...
    selection = [
        index
        for index, molecular_info in molecular_information.items()
        if (molecular_info["nb_atoms"] < 1500)
        and (molecular_info["stereochemistry"] == False)
        and (index not in remove_indices)
        and (scores[index]["valid"] == True)
    ]

    scores = {index: score for index, score in scores.items() if index in selection}
    molecular_information = {
        index: molecular_info
        for index, molecular_info in molecular_information.items()
