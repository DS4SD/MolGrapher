...
def main():
    ...
    plt.imshow(feature_map.detach(), alpha = 1,  cmap='jet', interpolation="None", extent=extent)
    plt.imshow(image, alpha=0.99*(image < 1), cmap='gray', extent=extent)
    plt.savefig(os.path.dirname(__file__) + "/../../../data/visualization/features_maps/" + image_name + "_sum.jpg")
    ...
    for plot_index in tqdm(range(nb_plots)):
        ...
        for i in range(nb_filters[0]):
            for j in range(nb_filters[1]):
                ...
                alphas = 1
                feature_map = ndimage.gaussian_filter(feature_map, sigma=(2, 2), order=0)
                ax[i, j].imshow(feature_map, alpha=alphas, cmap='hot_r', extent=extent)
                ax[i, j].imshow(image, alpha=0.99*(image < 1), cmap='gray', extent=extent)
                ax[i, j].set_title(str(feature_map_index))
                feature_map_index += 1
        plt.savefig(os.path.dirname(__file__) + "/../../../data/visualization/features_maps/" + image_name + "_" + str(plot_index) + ".jpg")
