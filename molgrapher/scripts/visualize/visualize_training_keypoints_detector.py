...
def main():
    ...
    if clean:
        if os.path.exists(os.path.dirname(__file__) + f"/../../../data/visualization/training_images/keypoint_detector/{output_folder}"):
            shutil.rmtree(os.path.dirname(__file__) + f"/../../../data/visualization/training_images/keypoint_detector/{output_folder}/")
    if not os.path.exists(os.path.dirname(__file__) + f"/../../../data/visualization/training_images/keypoint_detector/{output_folder}"):
        os.makedirs(os.path.dirname(__file__) + f"/../../../data/visualization/training_images/keypoint_detector/{output_folder}")
    ...
    for index in tqdm(range(min(len(loader)*config_dataset["batch_size"], max_index))):
        ...
        image, keypoints, image_filename, heatmap = sample["images"], sample["keypoints_batch"], sample["images_filenames"], sample["heatmaps_batch"]
        ...
        plt.savefig(os.path.dirname(__file__) + f"/../../../data/visualization/training_images/keypoint_detector/{output_folder}/{image_filename.split('/')[-1]}")
        plt.close()
