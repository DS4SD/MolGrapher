...
def visualize_predictions_maps(path, graph, graph_classifier, labels_colors, node_type, positions, image, classes_labels, ax, limit):
    ...
    predictions_confidences = [torch.max(prediction).item() for prediction in predictions[not node_type]]
    predictions = [classes_labels[torch.argmax(prediction).item()] for prediction in predictions[not node_type]]
    
    predicted_classes_positions = defaultdict(list)
    for index in range(len(positions)):
        predicted_classes_positions[predictions[index]].append({
            "position": positions[index],
            "confidence": predictions_confidences[index]
        })
    ...
    for predicted_class in predicted_classes_positions.keys():
        positions = [predicted_classes_positions[predicted_class][i]["position"] for i in range(len(predicted_classes_positions[predicted_class]))]
        confidences = [predicted_classes_positions[predicted_class][i]["confidence"] for i in range(len(predicted_classes_positions[predicted_class]))]
        mscatter(
            x = [position[0] for position in positions],
            y = [position[1] for position in positions],
            color = labels_colors[predicted_class],
            alpha = 0.3,
            m = ["o" if confidence > confidence_threshold else "." for confidence in confidences],
            ax = ax
        )
...
def main():
    ...
    for image_name in images_names:
        ...
        with open(os.path.dirname(__file__) + "/../../../data/config_dataset_benchmark.json") as file:
            config_dataset_benchmark = json.load(file)

        # Create dataset
        dataset = pd.DataFrame({
            "image_filename": [os.path.dirname(__file__) + f"/../../../data/benchmarks/uspto_large/images/" + image_name + ".png"],
        ...
        graph_classifier = GraphClassifier.load_from_checkpoint(
            os.path.dirname(__file__) + f"/../../../data/models/graph_classifier/{graph_classifier_model_name}.ckpt", 
        ...
        if atoms_predictions_map:
            ...
            visualize_predictions_maps(
                path = os.path.dirname(__file__) + "/../../../data/visualization/predictions_maps/" + image_name + "_atoms_predictions.jpg",
                graph = graph,
        ...
        if bonds_predictions_map:
            ...
            for atom_index in range(len(graph.atoms) - 1):
                ...
                positions.append([
                    (atoms_positions[atom_index][0] + atoms_positions[atom_index+1][0])//2,
                    (atoms_positions[atom_index][1] + atoms_positions[atom_index+1][1])//2
                ])

            visualize_predictions_maps(
                path = os.path.dirname(__file__) + "/../../../data/visualization/predictions_maps/" + image_name + "_bonds_predictions.jpg",
                graph = graph,
                graph_classifier = graph_classifier, 
        ...
        plt.savefig(os.path.dirname(__file__) + "/../../../data/visualization/predictions_maps/" + image_name + "_prediction_map.jpg")
