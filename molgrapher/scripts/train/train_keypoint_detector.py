...
def main():
    ...
    if mode == "fine-tuning":
        model = KeypointDetector.load_from_checkpoint(
            os.path.dirname(__file__) + f"/../../../data/models/keypoint_detector/{config_training['checkpoint']}.ckpt", 
    ...
    checkpoint = ModelCheckpoint(
        filename = os.path.dirname(__file__) + f"/../../../data/models/keypoint_detector/{config_dataset['experiment_name']}-{config_training['run_name']}-{{val_loss:.4f}}",
        save_top_k = 3, 
        monitor = "val_loss", 
        mode = 'min'
    )
    ...
    trainer = pl.Trainer(
        ...
        default_root_dir = os.path.dirname(__file__) + "/../../../data/logs/keypoint_detector/",
