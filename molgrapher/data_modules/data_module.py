...
class DataModule(pl.LightningDataModule):
    ...
    def _draw_process(self, images_filenames, smiles_list):
        keypoints_process = []
        for smiles, image_path in tqdm(
            zip(smiles_list, images_filenames), total=len(smiles_list)
        ):
            logger.info(smiles)
            try:
    ...
    def precompute_keypoints_synthetic(self):
        ...
        if self.config["nb_sample"] < dataset_triple_bonds_len:
            smiles_list = list(
                pd.read_csv(
                    ...
                    skiprows=sorted(
                        random.sample(
                            range(1, dataset_triple_bonds_len + 1),
                            dataset_triple_bonds_len - self.config["nb_sample"],
                        )
                    ),
                )["isosmiles"]
            )
        ...
        images_filenames = [
            images_folder_path + str(image_index) + ".png"
            for image_index in range(len(smiles_list))
        ]

        # Debug
        # keypoints_list = self._draw_process(images_filenames, smiles_list)

        images_filenames_split = np.array_split(
            images_filenames, self.config["num_processes_mp"]
        )
        smiles_list_split = np.array_split(smiles_list, self.config["num_processes_mp"])

        keypoints_list = []
        args = [
            [images_filenames_split[process_index], smiles_list_split[process_index]]
            for process_index in range(self.config["num_processes_mp"])
        ]
        ...
        molfiles_filenames = [
            image_filename[:-4] + ".mol" for image_filename in images_filenames
        ]
        self.save_images_filenames_keypoints(
            keypoints_list,
            images_filenames,
            molfiles_filenames,
            smiles_list,
    ...
    def get_cede_dataset(self):
        ...
        for cede_image_name in tqdm(labeled_data.images.index):
            ...
            annotation = labeled_data.annotations[
                labeled_data.annotations.image_id
                == labeled_data.images.loc[cede_image_name].id
            ]
    ...
    def setup(self, stage=None, hf_dataset=True):
        ...
        if (stage == TrainerFn.FITTING) and (
            self.config["training_dataset"] == "cede-synthetic"
        ):
            ...
            val_dataset, test_dataset = train_test_split(
                test_dataset, test_size=test_ratio / (test_ratio + val_ratio)
            )

            self.train_dataset = self.dataset_class(
                train_dataset, config=self.config, train=True
            )
            self.val_dataset = self.dataset_class(
                val_dataset, config=self.config, train=False
            )
            return
        ...
        if not hf_dataset:
            ...
            dataset = pd.DataFrame(
                {
                    "smiles": [
                        image["smiles"]
                        for index, image in enumerate(coco_json["images"])
                        if index < self.config["nb_sample"]
                    ],
                    "image_filename": [
            ...
            dataset["image_filename"] = [
                p.replace("joker", "molgrapher") for p in dataset["image_filename"]
            ]
            dataset["molfile_filename"] = [
                p.replace("joker", "molgrapher") for p in dataset["molfile_filename"]
            ]
            ...
            if self.clean_only:
                ...
                for index, molfile_filename in enumerate(dataset["molfile_filename"]):
                    ...
                    molfile_annotator = LabelMolFile(
                        molfile_filename, reduce_abbreviations=True
                    )
            ...
            train_filenames = pd.DataFrame(
                [p.split("/")[-1] for p in train_dataset["image_filename"]]
            )
            train_filenames.to_csv(
                os.path.dirname(__file__)
                + "/../../data/synthetic_images_molecules_keypoints/train_images_filenames_"
                + self.config["experiment_name"]
                + ".csv"
            )
            val_filenames = pd.DataFrame(
                [p.split("/")[-1] for p in val_dataset["image_filename"]]
            )
            val_filenames.to_csv(
                os.path.dirname(__file__)
                + "/../../data/synthetic_images_molecules_keypoints/val_images_filenames_"
                + self.config["experiment_name"]
                + ".csv"
            )
            test_filenames = pd.DataFrame(
                [p.split("/")[-1] for p in test_dataset["image_filename"]]
            )
            test_filenames.to_csv(
                os.path.dirname(__file__)
                + "/../../data/synthetic_images_molecules_keypoints/test_images_filenames_"
                + self.config["experiment_name"]
                + ".csv"
            )
        ...
        if hf_dataset:
            ...
            train_dataset["image_filename"] = [
                str(sample["id"]) + ".png" for _, sample in train_dataset.iterrows()
            ]
            train_dataset["molfile_filename"] = [
                str(sample["id"]) + ".mol" for _, sample in train_dataset.iterrows()
            ]
            val_dataset["image_filename"] = [
                str(sample["id"]) + ".png" for _, sample in val_dataset.iterrows()
            ]
            val_dataset["molfile_filename"] = [
                str(sample["id"]) + ".mol" for _, sample in val_dataset.iterrows()
            ]

            self.train_dataset = self.dataset_class(
                train_dataset, config=self.config, train=True, hf_dataset=hf_dataset
            )
            self.val_dataset = self.dataset_class(
                val_dataset,
                config=self.config,
    ...
    def save_images_filenames_keypoints(
        self,
        keypoints_list,
        images_filenames,
        molfiles_filenames,
        ...
        # Save images
        for image_index, (keypoints, image_filename, molfile_filename, smiles) in tqdm(
            enumerate(
                zip(keypoints_list, images_filenames, molfiles_filenames, smiles_list)
            ),
    ...
    def precompute_keypoints_benchmarks(self, dataset_max_index, dataset):
        ...
        images_folder_path = (
            os.path.dirname(__file__) + "/../../data/benchmarks/" + dataset + "/images/"
        )
        molfiles_folder_path = (
            os.path.dirname(__file__)
            + "/../../data/benchmarks/"
            + dataset
            + "/molfiles/"
        )
        synthetic_images_folder_path = (
            os.path.dirname(__file__)
            + "/../../data/benchmarks/"
            + dataset
            + "/synthetic_images/"
        ...
        save_path = (
            ...
            + "/../../data/benchmarks/"
            + dataset
            + "/keypoints_images_filenames_"
            + self.config["experiment_name"]
            + "_"
            + str(dataset_max_index)
            + "_clean_"
            + str(int(self.clean_only))
            + ".json"
        )
        ...
        for molfile_filename in glob.glob(molfiles_folder_path + "*"):
            ...
            image_path = (
                images_folder_path + molfile_filename[:-4].split("/")[-1] + ".png"
            )
            ...
            if not path.exists(image_path):
                print(
                    f"No image corresponding to the molfile {molfile_filename} was found"
                )
                continue
        ...
        keypoints_list = []
        remove_indices = []
        for dataset_index, (molfile_filename, image_path) in tqdm(
            enumerate(zip(molfiles_filenames, images_filenames)),
            total=len(molfiles_filenames),
        ):
            # Compute keypoints
            synthetic_image_path = (
                synthetic_images_folder_path
                + molfile_filename[:-4].split("/")[-1]
                + ".png"
            )
            ...
            try:
                ...
                print(
                    "Keypoints generation error: The molecule property cache can not be updated"
                )
            ...
            try:
                # Manual Cleaning
                if any(
                    (name in synthetic_image_path)
                    for name in [
                        "US20220281806A1-20220908-C00012",
                        "US20220281806A1-20220908-C00019",
                    ]
                ):
                    remove_indices.append(dataset_index)
                    continue
                ...
                print(
                    "Keypoints generation error: The molecule can not be drawn using RDKit"
                )
                remove_indices.append(dataset_index)
                continue
        ...
        molfiles_filenames = [
            molfile_filename
            for i, molfile_filename in enumerate(molfiles_filenames)
            if i not in remove_indices
        ]

        keypoints_list_transformed = []
        remove_indices = []
        for dataset_index, (molfile_filename, image_filename) in tqdm(
            enumerate(zip(molfiles_filenames, images_filenames)),
            total=len(molfiles_filenames),
        ):

            synthetic_image_filename = (
                synthetic_images_folder_path
                + molfile_filename[:-4].split("/")[-1]
                + ".png"
            )
            ...
            if transformation is not None:
                ...
                keypoints_transformed = self.transform_keypoints(
                    keypoints, transformation, margin
                )
            else:
            ...
            for keypoint in keypoints_transformed:
                ...
                if (image_crop != 255.0).sum() < 1:
                    empty_keypoint = True
                    break

            if empty_keypoint:
                print(
                    f"Tranformation discarded {dataset_index}. Some aligned keypoints match empty image regions."
                )
                remove_indices.append(dataset_index)
                continue
        ...
        print(
            f"Proportion of removed samples in {dataset}: {round(len(remove_indices)/len(images_filenames), 4)}"
        )
        images_filenames = [
            image_filename
            for i, image_filename in enumerate(images_filenames)
    ...
    def setup_keypoints_benchmarks(self, stage="test"):
        ...
        if stage == "fine-tuning":
            ...
            save_path = (
                os.path.dirname(__file__)
                + "/../../data/benchmarks/"
                + self.config["training_dataset_fine-tuning"]
                + "/keypoints_images_filenames_"
                + self.config["experiment_name"]
                + "_"
                + str(dataset_max_index)
            ...
            if (not os.path.exists(save_path)) or self.force_precompute:
                print(
                    f"Precompute keypoints for fine-tuning training dataset: {self.config['training_dataset_fine-tuning']}"
                )
                self.precompute_keypoints_benchmarks(
                    dataset_max_index, self.config["training_dataset_fine-tuning"]
                )
            ...
            dataset = pd.DataFrame(
                {
                    "smiles": smiles_list,
                    "image_filename": images_filenames,
                    "molfile_filename": molfiles_filenames,
            ...
            # Temporary fix
            images_filenames = [
                p.replace("joker", "molgrapher") for p in images_filenames
            ]
            molfiles_filenames = [
                p.replace("joker", "molgrapher") for p in molfiles_filenames
            ]
        ...
        for benchmark, dataset_max_index in zip(
            self.config["benchmarks"], self.config["nb_sample_benchmarks"]
        ):
            save_path = (
                os.path.dirname(__file__)
                + "/../../data/benchmarks/"
            ...
            self.benchmarks_datasets.append(
                self.dataset_class(
                    dataset,
                    config=self.config,
                    train=False,
    ...
    def setup_molfiles_benchmarks_static(self, benchmark, nb_sample_benchmark):
        ...
        images_filenames = [
            ...
            if (index < nb_sample_benchmark) and (index > min_index)
        ]
        molfiles_filenames = [
            molfile_filename
            for index, molfile_filename in enumerate(molfiles_filenames)
            if (index < nb_sample_benchmark) and (index > min_index)
        ]
        smiles_list = [
            None for index, molfile_filename in enumerate(molfiles_filenames)
        ]
        dataset = pd.DataFrame(
            {
                "smiles": smiles_list,
    ...
    def setup_images_benchmarks(self, max_index=None):
        ...
        if isinstance(self.images_or_paths, str) or isinstance(
            ...
            if all(
                isinstance(image, str) or isinstance(image, Path)
                for image in self.images_or_paths
            ):
                images_filenames = self.images_or_paths
            else:
                images_filenames = [
                    f"{i}.png" for i in range(len(self.images_or_paths))
                ]
    ...
    def preprocess(self):
        ...
        for benchmark_dataset in self.benchmarks_datasets:
            self.caption_remover = CaptionRemover(
                self.config,
                force_cpu=self.force_cpu,
                remove_captions=self.remove_captions,
            )
            if self.has_pil_images:
                preprocessed_images = self.caption_remover.preprocess_images(
                    benchmark_dataset.dataset["image"]
                )
                benchmark_dataset.dataset["image"] = preprocessed_images
            else:
                preprocessed_images = self.caption_remover.preprocess_images(
                    benchmark_dataset.dataset["image_filename"]
                )
    ...
    def transform_keypoints(self, keypoints, transformation, margin):
        keypoints = [
            [keypoint[0] + margin, keypoint[1] + margin] for keypoint in keypoints
        ]
        transformed_keypoints = cv2.transform(np.array([keypoints]), transformation)[0]
        transformed_keypoints = [
            [int(keypoint[0] - margin), int(keypoint[1] - margin)]
            for keypoint in transformed_keypoints
        ]
    ...
    def val_dataloader(self):
        ...
        if self.mode == "fine-tuning":
            ...
            print(
                f"Synthetic validation set length: {len(self.val_dataset)} (1/10 of total dataset size)"
            )
            print(
                f"Real validation set length: {len(self.val_dataset_real)} (1/10 of total dataset size)"
            )
            print(
