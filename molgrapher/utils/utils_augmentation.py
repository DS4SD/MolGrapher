    ...
    transforms_list.append(
        albu.OneOf(
            [
                ...
                GaussianBlur(blur_limit=21, p=0.25),
                albu.Compose(
                    [
                        Affine(
                        ...
                        Resize(config["image_size"][1], config["image_size"][2]),
                    ],
                    p=0.25,
                ),
            ],
    ...
    transforms_list.append(
        albu.OneOf(
            [
                ...
                GaussianBlur(blur_limit=11, p=0.25),
                albu.Compose(
                    [
                        Affine(
                            shear=({"x": (-20, 20), "y": (-20, 20)}),
    ...
    transforms_list.append(
        albu.OneOf(
            [
                ...
                GaussianBlur(blur_limit=51, p=0.25),
                albu.Compose(
                    [
                        Affine(
...
class GraphTransformer:
    def __init__(
        self,
        config,
        keypoints_shift_limit,
        decoy_keypoint_shift_limit,
        decoy_atom_population_density,
    ...
    def shift_keypoints_positions(self, keypoints, shift_window):
        ...
        for keypoint in keypoints:
            ...
            keypoint = [
                max(
                    min(
                        int(center_x + radius * np.cos(theta)),
                        self.config["image_size"][1] - 1,
                    ),
                    shift_window[0],
                ),
                max(
                    min(
                        int(center_y + radius * np.sin(theta)),
                        self.config["image_size"][1] - 1,
                    ),
                    shift_window[0],
                ),
    ...
    def shift_atoms_positions(self, graph):
        shift_window = (
            self.keypoints_shift_limit[0] * graph.bond_size,
            self.keypoints_shift_limit[1] * graph.bond_size,
        )
    ...
    def add_decoy_atom(self, graph):
        shift_window = (
            self.decoy_keypoint_shift_limit[0] * graph.bond_size,
            self.decoy_keypoint_shift_limit[1] * graph.bond_size,
        )
        ...
        keypoints = self.shift_keypoints_positions(
            [[atom["position"][0], atom["position"][1]]], shift_window
        )
