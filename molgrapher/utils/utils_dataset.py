...
class CaptionRemover:
    ...
    def __call__(self, pil_image):
        ...
        if len(remove_indices) != 0:
            remove_boxes = [
                self.boxes[i] for i in range(len(self.boxes)) if i in remove_indices
            ]
    ...
    def detect_strange_captions(self):
        ...
        for i in range(len(self.texts)):
            number_of_characters_allowed = len(
                [
                    substring
                    for substring in self.texts[i]
                    if substring in characters_allowed
                ]
            )
            ...
            elif (len(self.texts[i]) > 2) and (
                number_of_characters_allowed < len(self.texts[i]) / 2
            ):
                filter_masks = True
            elif (
                (self.texts[i].isdigit())
                and (int(self.texts[i]) != 0)
                and (self.scores[i] >= 0.85)
            ):
                filter_masks = True
            elif (len(self.texts[i]) > 2) and (self.texts[i].count(",") > 2):
                filter_masks = True
            elif (len(self.texts[i]) == 2) and any(
                substring in self.texts[i] for substring in ["(", ")", ":"]
            ):
                filter_masks = True
        ...
        for i in range(len(contours)):
            if i != max_index:
                ...
                self.image = cv2.drawContours(
                    self.image, [approx], 0, (255, 255, 255), thickness=cv2.FILLED
                )
    ...
    def generate_masks(self):
        ...
        dilated_mask = cv2.dilate(
            mask, cv2.getStructuringElement(cv2.MORPH_RECT, (13, 13)), iterations=2
        )
        flood_fill_margin = 0
        ...
        flood_fill_mask = np.zeros(
            (dilated_mask.shape[0] + 2, dilated_mask.shape[1] + 2), np.uint8
        )
        cv2.floodFill(dilated_mask, flood_fill_mask, (0, 0), 255)
        is_black_pixel = np.where(
            (dilated_mask[:, :, 0] == 0)
            & (dilated_mask[:, :, 1] == 0)
            & (dilated_mask[:, :, 2] == 0)
        )
