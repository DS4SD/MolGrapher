...
class GraphPostprocessor:
    def __init__(
        ...
        filename_logging="",
        molecule=None,
        needs_update_stereo=None,
        bonds_directions=None,
    ):
    ...
    def postprocess_after_rdkit_molecule_creation(self):
        ...
        if (
            all(
                [
                    (self.molecular_graph.atoms_classes_symbols[atom["class"]] != "R")
                    for atom in self.molecular_graph.atoms
                ]
            )
            and len(Chem.GetMolFrags(self.molecule)) == 1
        ):
            # print(f"{self.filename_logging}:Return molecule without abbreviations.")
            if self.align_rdkit_output:
                try:
                    rdDepictor.GenerateDepictionMatching2DStructure(
                        self.molecule, reference=molecule_aligned
                    )
                    recompute_coords = False
                except Exception as e:
                    print(
                        f"{self.filename_logging}:Aligning RDKit molecule failed: {e}"
                    )
            ...
            molecule_return = self.try_sanitize_molecule(
                self.molecule, recompute_coords=recompute_coords
            )
            if molecule_return:
        ...
        if len(Chem.GetMolFrags(self.molecule)) == 1:
            ...
            if self.align_rdkit_output:
                ...
                for abbreviation_index in sorted(
                    self.matched_abbreviations_indices, reverse=True
                ):
                    molecule_aligned_2.RemoveAtom(abbreviation_index)
                molecule_aligned_2 = molecule_aligned_2.GetMol()
                try:
                    rdDepictor.GenerateDepictionMatching2DStructure(
                        self.molecule, reference=molecule_aligned_2
                    )
                    recompute_coords = False
                except Exception as e:
                    print(
        ...
        elif ("postprocessed_keypoints" not in self.postprocessing_flags) and (
            len(self.original_abbreviations) > 0
        ):
            output = self.merge_keypoints()
            if output is not None:
                print(
                    f"{self.filename_logging}:Recursive molecule creation (after merging keypoints using PaddleOCR predicted boxes)"
                )
                return output
        ...
        if len(Chem.GetMolFrags(original_molecule)) == 1:
            print(
                f"{self.filename_logging}:Merging keypoints failed. Return original molecule."
            )
            if self.align_rdkit_output:
                molecule_aligned_2 = Chem.EditableMol(molecule_aligned)
                for abbreviation_index in sorted(
                ...
                recompute_coords = True
            original_molecule_return = self.try_sanitize_molecule(
                original_molecule, recompute_coords=recompute_coords
            )
            if original_molecule_return:
                return original_molecule_return

        # Remove single atom without connections (Graph post-processing)
        if ("postprocessed_isolated_atoms" not in self.postprocessing_flags) and (
            len(Chem.GetMolFrags(original_molecule)) > 1
        ):
            output = self.remove_isolated_atoms()
            if output is not None:
                print(
                    f"{self.filename_logging}:Recursive molecule creation (after removing isolated atoms)"
                )
                return output
    ...
    def try_sanitize_molecule(self, molecule, recompute_coords=False):
        try:
            ...
            print(
                f"{self.filename_logging}:The predicted molecule can not be sanitized: {e}"
            )
            return False
        ...
        for bond in self.molecule.GetBonds():
            try:
                if (str(bond.GetBondType()) == "AROMATIC") and (not bond.IsInRing()):
                    ...
                    self.molecule.GetAtomWithIdx(bond.GetBeginAtomIdx()).SetIsAromatic(
                        False
                    )
                    self.molecule.GetAtomWithIdx(bond.GetEndAtomIdx()).SetIsAromatic(
                        False
                    )
            except Exception as e:
    ...
    def postprocess_aromatic_rings(self):
        ...
        for bonds_ring in bonds_rings:
            ...
            if (most_common_type == "AROMATIC") and any(
                [type != "AROMATIC" for type in types]
            ):
                for bond in bonds_ring:
                    self.molecule.GetBondWithIdx(bond).SetBondType(
    ...
    def resolve_abbreviations(self):
        ...
        for abbreviation_index, atom in enumerate(self.molecular_graph.atoms):
            ...
            for i, abbreviation in enumerate(self.abbreviations):
                if not (
                    (atom["position"][0] >= (abbreviation["box"][0][0]))
                    and (atom["position"][0] <= (abbreviation["box"][1][0]))
                    and (atom["position"][1] >= (abbreviation["box"][0][1]))
                    and (atom["position"][1] <= (abbreviation["box"][1][1]))
                ):
                    continue
                # Remove the matched abbreviation
                self.abbreviations = [
                    abb for i_abb, abb in enumerate(self.abbreviations) if (i_abb != i)
                ]

                if (abbreviation["text"] not in self.abbreviations_smiles_mapping) and (
                    abbreviation["text"] not in self.ocr_atoms_classes_mapping
                ):
                    ...
                    Chem.SetAtomAlias(
                        self.molecule.GetAtomWithIdx(abbreviation_index),
                        f"{abbreviation['text']}",
                    )
                ...
                if abbreviation["text"] in self.ocr_atoms_classes_mapping:
                    ...
                    atom_symbol = self.ocr_atoms_classes_mapping[abbreviation["text"]][
                        "symbol"
                    ]
                    ...
                    if "," in atom_symbol:
                        ...
                        molecule_editable.ReplaceAtom(
                            abbreviation_index, Chem.Atom(atom_symbol)
                        )
                ...
                if abbreviation["text"] not in self.abbreviations_smiles_mapping:
                    self.molecule.GetAtomWithIdx(abbreviation_index).SetProp(
                        "atomLabel", f"[{abbreviation['text']}]"
                    )
                    Chem.SetAtomAlias(
                        self.molecule.GetAtomWithIdx(abbreviation_index),
                        f"[{abbreviation['text']}]",
                ...
                molecule_abbreviation = Chem.MolFromSmiles(
                    self.abbreviations_smiles_mapping[abbreviation["text"]]["smiles"]
                )
                if molecule_abbreviation is None:
                    print(
                        f"{self.filename_logging}:Invalid abbreviation mapping: {abbreviation}, {self.abbreviations_smiles_mapping[abbreviation['text']]['smiles']}"
                    )
                    continue
                ...
                for atom_index, connection_atom in enumerate(
                    molecule_abbreviation.GetAtoms()
                ):
                    if connection_atom.GetSymbol() == "*":
                        bonds = connection_atom.GetBonds()
                        if len(bonds) > 1:
                            print(
                                f"{self.filename_logging}:Error: connection atom from abbreviation has multiple bonds"
                            )
                        ...
                        for neighbor_index in [
                            ...
                            if molecule_abbreviation.GetAtomWithIdx(
                                neighbor_index
                            ).HasProp(f"{abbreviation_index}-attachmentIndex"):
                                # Different atoms of the molecule can attach to the same position in the abbreviation
                                molecule_abbreviation.GetAtomWithIdx(
                                    neighbor_index
                                ).SetProp(
                                    f"{abbreviation_index}-attachmentIndex",
                                    molecule_abbreviation.GetAtomWithIdx(
                                        neighbor_index
                                    ).GetProp(f"{abbreviation_index}-attachmentIndex")
                                    + ","
                                    + str(connection_point_abbreviation_index),
                                )
                            else:
                                molecule_abbreviation.GetAtomWithIdx(
                                    neighbor_index
                ...
                nb_connection_points_abbreviation_molecule = (
                    connection_point_abbreviation_index
                )
                ...
                for atom_index in range(
                    molecule_abbreviation.GetNumAtoms() - 1, -1, -1
                ):
                    if (
                        molecule_abbreviation.GetAtomWithIdx(atom_index).GetSymbol()
                        == "*"
                    ):
                ...
                for atom_index, connection_atom in enumerate(
                    ...
                    if connection_atom.HasProp(f"{abbreviation_index}-attachmentIndex"):
                        connection_point_abbreviation_indices = connection_atom.GetProp(
                            f"{abbreviation_index}-attachmentIndex"
                        ).split(",")
                        for (
                            connection_point_abbreviation_index
                        ) in connection_point_abbreviation_indices:
                            molecule_abbreviation_connection_points[
                                connection_point_abbreviation_index
                            ]["index"] = atom_index
                ...
                for connection_atom in self.molecule.GetAtomWithIdx(
                    abbreviation_index
                ).GetNeighbors():
                    if connection_atom.HasProp("atomLabel") and (
                        connection_atom.GetProp("atomLabel") == "abbreplaced"
                    ):
                        continue
                ...
                if (
                    nb_connection_points_molecule
                    != nb_connection_points_abbreviation_molecule
                ):
                    print(
                        f"{self.filename_logging}:The number of connection point between the predicted abbreviation node and the associated sub-molecule \
                ...
                for connection_atom in self.molecule.GetAtomWithIdx(
                    ...
                    connection_atom.SetProp(
                        f"{abbreviation_index}-attachmentIndex",
                        str(connection_point_index),
                    )
                    connection_point_index += 1
                ...
                for atom_index, connection_atom in enumerate(self.molecule.GetAtoms()):
                    if connection_atom.HasProp(f"{abbreviation_index}-attachmentIndex"):
                        connection_point_index = connection_atom.GetProp(
                            f"{abbreviation_index}-attachmentIndex"
                        )
                ...
                for connection_point_index in molecule_connection_points.keys():
                    try:
                        molecule_editable.AddBond(
                            ...
                            + offset,
                            order=molecule_abbreviation_connection_points[
                                connection_point_index
                            ]["bond"],
                        )
                    except Exception as e:
                        print(
                            f"{self.filename_logging}:Error multiple connection points abbreviation ({connection_point_index}, {abbreviation}): {e}"
                        )

                self.molecule = molecule_editable.GetMol()
                self.molecule.GetAtomWithIdx(abbreviation_index).SetProp(
                    "atomLabel", "abbreplaced"
                )
    ...
    def remove_isolated_atoms(self):
        ...
        atoms_involved_in_connections = list(
            set(
                [bond["index"][0] for bond in self.molecular_graph.bonds]
                + [bond["index"][1] for bond in self.molecular_graph.bonds]
            )
        )
                    ...
                    atoms_new.append(
                        {
                            ...
                            "position": self.molecular_graph.atoms[atom_idx][
                                "position"
                            ],
                            "type": 1,
                        }
    ...
    def merge_keypoints(self):
        ...
        for i, abbreviation in enumerate(self.original_abbreviations):
            ...
            if abbreviation["text"] in self.abbreviations_smiles_mapping:
                ...
                atom["class"] == atoms_matched[0]["class"] for atom in atoms_matched
            ):
            ...
            atoms_matched_indices = [
                atom_index
                for atom_index, atom in enumerate(self.molecular_graph.atoms)
                if atom in atoms_matched
            ]
            for bond_index, bond in enumerate(self.molecular_graph.bonds):
                if (bond["index"][0] in atoms_matched_indices) and (
                    bond["index"][1] in atoms_matched_indices
                ):
                ...
                if bond["index"][0] in atoms_matched_indices:
                    ...
                    if all(
                        (new_index != bond["index"])
                        for bond in self.molecular_graph.bonds
                    ) and all(
                        (list(reversed(new_index)) != bond["index"])
                        for bond in self.molecular_graph.bonds
                    ):
            ...
            self.molecular_graph.bonds = [
                bond
                for bond_index, bond in enumerate(self.molecular_graph.bonds)
                if bond_index not in remove_bonds_indices
            ]
            ...
            for i, bond in enumerate(self.molecular_graph.bonds):
                self.molecular_graph.bonds[i]["index"][0] -= sum(
                    bond["index"][0] > removed_atom_index
                    for removed_atom_index in atoms_matched_indices
                )
                self.molecular_graph.bonds[i]["index"][1] -= sum(
                    bond["index"][1] > removed_atom_index
                    for removed_atom_index in atoms_matched_indices
                )
    ...
    def assign_stereo_centers(self, molecule):
        ...
        self.bonds_directions[0] = [
            (bond[0] + 1, bond[1] + 1) for bond in self.bonds_directions[0]
        ]
                ...
                for l in molfile.readlines():
                    ...
                    try:
                        ...
                        if (int(sl[0]), int(sl[1])) in self.bonds_directions[0]:
                            bond_index = self.bonds_directions[0].index(
                                (int(sl[0]), int(sl[1]))
                            )
        ...
        edited_molecule = Chem.MolFromMolFile(
            edited_molfile_path, sanitize=False, removeHs=False
        )
