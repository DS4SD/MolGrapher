...
def get_molecule_information(smiles=None, molfile_filename=None):
    ...
    if molfile_filename:
        molecule = get_molecule_from_molfile(
            molfile_filename, remove_stereochemistry=False
        )
        if molecule:
    ...
    molecule_atoms = [
        atom.replace("[", "").replace("]", "").replace("H", "").replace("@", "")
...
def compute_molecule_prediction_quality(
    ...
    if not predicted_molecule:
        predicted_molecule = get_molecule_from_smiles(
            predicted_smiles, remove_stereochemistry=remove_stereo
        )
    if not gt_molecule:
        gt_molecule = get_molecule_from_smiles(
            gt_smiles, remove_stereochemistry=remove_stereo
        )
    ...
    # Inchi equality
    if remove_stereo:
        if MolToInchi(predicted_molecule, options="/SNon") == MolToInchi(
    ...
    ).score(" ".join([c for c in gt_smiles]), " ".join([c for c in predicted_smiles]))
...
def get_metrics(predictions, selection):
    ...
    gt_atoms = [
        prediction
        for index, prediction in enumerate(predictions["gt_nodes"])
        if index in selection
    ]
    predicted_atoms = [
        prediction
        for index, prediction in enumerate(predictions["predictions_nodes"])
        if index in selection
    ]
    gt_bonds = [
        prediction
        for index, prediction in enumerate(predictions["gt_edges"])
        if index in selection
    ]
    predicted_bonds = [
        prediction
        for index, prediction in enumerate(predictions["predictions_edges"])
        if index in selection
    ]
    ...
    metrics = {
        "atoms": {
            "confusion_matrix": confusion_matrix(
                gt_atoms_flat,
                predicted_atoms_flat,
                labels=list(vocabulary_atoms.values()),
            ).tolist(),
            "classification_report": classification_report(
                gt_atoms_flat,
                predicted_atoms_flat,
                output_dict=True,
                target_names=list(vocabulary_atoms.keys()),
                labels=list(vocabulary_atoms.values()),
            ),
        },
        "bonds": {
            ...
            "classification_report": classification_report(
                ...
                predicted_bonds_flat,
                output_dict=True,
                target_names=list(vocabulary_bonds.keys()),
                labels=list(vocabulary_bonds.values()),
            ),
        },
        "molecules": {
            "atoms": accuracy_score(
                binarizer.transform(gt_atoms), binarizer.transform(predicted_atoms)
            ),
            "bonds": accuracy_score(
                binarizer.transform(gt_bonds), binarizer.transform(predicted_bonds)
            ),
        },
    }
...
def get_metrics_keypoint_detector(gt_keypoints, predicted_keypoints, test_window=10):
    ...
    for gt_i, gt_keypoint in enumerate(gt_keypoints):
        others_gt_keypoints = gt_keypoints[
            [i for i in range(len(gt_keypoints)) if (i != gt_i)]
        ]
        if bops.box_iou(gt_keypoint.unsqueeze(0), others_gt_keypoints).sum() > 0:
            print(
                "ERROR in get_metrics_keypoint_detector: Some of the ground truth keypoints are too close."
            )
            return None, None
    tp = 0
    for gt_keypoint in gt_keypoints:
        for predicted_keypoint in predicted_keypoints:
            if (
                bops.box_iou(
                    predicted_keypoint.unsqueeze(0), gt_keypoint.unsqueeze(0)
                ).item()
                > 0
            ):
                tp += 1
